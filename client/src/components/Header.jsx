...
import { React, useState } from 'react';
import { BsJustifyRight, BsMessenger } from 'react-icons/bs';
import { Link, useNavigate } from 'react-router-dom';
import MobileMenu from './mobileMenu';
import { useDispatch, useSelector } from 'react-redux';
import Profile from './ProfileOption';
...
const Header = () => {
    ...
    return (
        <>
            <div className="navbar pl-0 pr-0 pt-4 pb-4 bg-black shadow-md text-white"> {/* Increased padding */}
                <div className="px-5 max-w-screen-2xl w-full !mx-auto grid grid-cols-12 gap-1 items-center">
                    {/* Logo and Branding */}
                    <div className="col-span-3 flex justify-start">
                        <h1 className="sm:text-xl text-sm text-left text-white tracking-tighter font-heading font-bold flex items-center">
                            <Link to="/" className="flex items-center ml-10 justify-start">
                                <span className="hidden text-white sm:block ml-2">TaskLINK</span>
                            </Link>
                        </h1>
                    </div>
                    ...
                    <div className="col-span-6 flex justify-center">
                        <ul className="hidden sm:flex space-x-6 text-white font-semibold">
                            ...
                            <li><Link to="/search" className="text-white hover:text-gray-300">Listings</Link></li>
                    ...
                    <div className="col-span-3 flex justify-end items-center">
                        <ul className="hidden sm:flex items-center space-x-4">
                            <li className="relative">
                                <Link to={`${currentUser ? "/message" : "/login"}`}>
                                    <BsMessenger className="text-xl text-white hover:text-gray-300" />
                                    {notificationsDB.length > 0 && (
                                        <span className="absolute top-0 right-0 bg-red-600 text-xs text-white px-1 rounded-full">
                            ...
                            {currentUser ? (
                                ...
                                <li>
                                    <Link to="/login" className="px-4 py-2 bg-white text-black font-bold rounded hover:bg-gray-200">
                                        Login/Register
                                    </Link>
                                </li>
                            )}
                        ...
                        >
                            {isActiveMobile ? <MdOutlineClose /> : <BsJustifyRight />}
                        </button>
                    </div>
                </div>
            </div>
            ...
            {/* Mobile Menu */}
            {isActiveMobile && <MobileMenu menuStatus={{ isActiveMobile, setIsActiveMobile }} />}
        </>
    );
};
