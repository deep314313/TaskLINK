...
const Profile = () => {
  ...
  const handleFileUpload = (file) => {
    if (file) {
      ...
      formData.append('upload_preset', 'task_link'); // Replace with your Cloudinary upload preset
      formData.append('cloud_name', 'dwtbjre3q'); // Replace with your Cloudinary cloud name
  
      const uploadUrl = 'https://api.cloudinary.com/v1_1/dwtbjre3q/image/upload'; // Cloudinary API URL
      ...
      xhr.upload.addEventListener('progress', (event) => {
        if (event.lengthComputable) {
          ...
          setUploadingPerc(Math.round(progress)); // Update the progress state
        }
      });
      ...
      xhr.addEventListener('load', () => {
        if (xhr.status === 200) {
          ...
          setFormData({ ...formData, avatar: response.secure_url }); // Save the URL of the uploaded file
        } else {
      ...
      xhr.addEventListener('error', () => {
        setFileUploadError(true); // Handle network error
      });
          ...
          <div className="profile_info p-5 bg-white  w-full  md:col-span-2 lg:col-span-1  md:max-h-full md:min-h-screen h-full col-span-1 ">
            ...
            <form className='w-full' onSubmit={handleSubmit} >
              <div className='mb-3'>
                <div className="image_container w-full flex items-center justify-center  relative max-w-[100px] mx-auto">
                  ...
                  <input onChange={(e) => setFile(e.target.files[0])}
                    ...
                    ref={fileRef}
                  />
                  <img src={formData.avatar || currentUser.avatar} onClick={() => fileRef.current.click()} className='h-24 w-24 object-cover mb-3 rounded-full border-[1px]  border-black' alt="profile image" />
                  <i className='h-6 w-6 rounded-full flex items-center justify-center bg-black text-white absolute bottom-3 right-0'><AiFillEdit /></i>
                </div>
              ...
              <input
                ...
                className="form_input bg-slate-200 rounded-md !pl-3 mt-1 !border-[1px] focus:!border-black mb-3"
              ...
              <input
                ...
                className="  form_input bg-slate-200 rounded-md !pl-3 !border-[1px] focus:!border-black mb-3"
                onChange={handleChange}
              />
              ...
              <input
                ...
                className="mt-1  form_input bg-slate-200 rounded-md !pl-3 !border-[1px] focus:!border-black"
                onChange={handleChange}
              />

              <button disabled={loading} type='submit' className='py-2 px-5 bg-black text-white rounded-md w-full font-heading  mt-4 hover:opacity-90'>
                {
